...
function resolveProjectId(u) {
  ...
  return safeId(q) || safeId(activeProjectId());
}

function mediaDir(projectId, owner) {
  return join(PROJECTS_ROOT, projectId, 'media', owner);
}
...
function readIndex(projectId, owner) {
  ...
  try {
    ...
    return items.filter((it) => it && it.id && it.file && existsSync(join(dir, it.file)));
  } catch {
    return [];
  }
}
...
export async function handleW28Api(req, res, u, parts, helpers) {
  ...
  if (action === 'file' && (req.method === 'GET' || req.method === 'HEAD')) {
    ...
    res.setHeader(
      'Content-Disposition',
      `${u.searchParams.get('download') ? 'attachment' : 'inline'}; filename="${downloadName.replace(/"/g, '')}"; filename*=UTF-8''${encodeURIComponent(downloadName)}`
    );
  ...
  if ((action === 'delete' || action === 'rename') && req.method === 'POST') {
    ...
    const projectId = safeId(body.project_id) || safeId(activeProjectId());
