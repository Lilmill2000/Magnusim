 ...
 *
 * Setup writes ``.magnusim-local.json`` (also reads legacy ``.cfddesk-local.json``).
 * Until then we read MAGNUSIM_WSL_* / CFDDESK_WSL_* env vars and, if needed, ask
 * WSL for $HOME/cases so a clone does not write under a previous machine's home.
 */
...
export function wslSettings() {
  ...
  const distro =
    String(envGet('WSL_DISTRO') || '').trim() ||
    String(local.wsl_distro || '').trim() ||
    DEFAULT_WSL_DISTRO;
  let root =
    String(envGet('WSL_CASE_ROOT') || '').trim() ||
...
/** Forget a cached probe (Setup just wrote local json). */
...
/**
 * Once per Vite process: warn if local json lacks OpenFOAM/cfMesh
...
export function verifyWslToolchain() {
  ...
  if (!recordedOf) {
    console.warn('[magnusim] local json missing openfoam_version — re-run Setup.bat to pin toolchain');
  }
  if (!recordedCf && String(local.cartesianMesh || '') === 'yes') {
    console.warn('[magnusim] local json missing cfmesh_version');
  }
  try {
    const r = spawnSync(
      'wsl',
    ...
    if (r.status !== 0) {
      console.warn('[magnusim] could not probe foamVersion in WSL distro', distro);
      return;
    }
    ...
    if (recordedOf && live && recordedOf !== live && !live.includes(recordedOf) && !recordedOf.includes(live)) {
      console.warn('[magnusim] OpenFOAM version drift: local json has ' + recordedOf + ' but WSL reports ' + live);
    }
  } catch (err) {
    console.warn('[magnusim] verifyWslToolchain failed', err && err.message);
  }
}
